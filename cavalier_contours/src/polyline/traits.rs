...
use super::{
    ...
    PlineIntersectsCollection, PlineOrientation, PlineSegIterator, PlineSelfIntersectOptions,
...
pub trait PolylineRef<'a>: std::ops::Index<usize, Output = PlineVertex<Self::Num>> {
    type Num: Real;
    type SegmentIter: Iterator<Item = (PlineVertex<Self::Num>, PlineVertex<Self::Num>)> + Clone + 'a;
    type VertexIter: Iterator<Item = PlineVertex<Self::Num>> + Clone + 'a;
    type OutputPolyline: PolylineRefMut<'a, Num = Self::Num> + PolylineCreation<Num = Self::Num>;
    ...
    fn remove_repeat_pos(&'a self, pos_equal_eps: Self::Num) -> Option<Self::OutputPolyline> {
        ...
        let mut prev_pos = self.get(0).unwrap().pos();
        ...
        if self.is_closed()
            && self
                ...
                .unwrap()
                .pos()
                .fuzzy_eq_eps(self.get(0).unwrap().pos(), pos_equal_eps)
        {
            result
                .get_or_insert_with(|| {
    ...
    fn remove_redundant(&'a self, pos_equal_eps: Self::Num) -> Option<Self::OutputPolyline> {
        ...
        if vc == 2 {
            if self
                ...
                .unwrap()
                .pos()
                .fuzzy_eq_eps(self.get(1).unwrap().pos(), pos_equal_eps)
            {
                let mut result = Self::OutputPolyline::with_capacity(1, self.is_closed());
                result.add_vertex(self.get(0).unwrap());
                return Some(result);
            }
            return None;
        }
        ...
        if self.is_closed() {
            ...
            match result.as_mut() {
                Some(pl) => {
                    if pl
                        ...
                        .unwrap()
                        .pos()
                        .fuzzy_eq_eps(pl.get(0).unwrap().pos(), pos_equal_eps)
                    {
                        pl.remove_last();
                    }
                }
            ...
            let v3 = match result.as_ref() {
                Some(pl) => pl.get(1).unwrap(),
                None => self.get(1).unwrap(),
            };
            ...
            match result.as_mut() {
                ...
                None => {
                    if self
                        .get(vc - 2)
                        .unwrap()
                        .fuzzy_eq_eps(self.get(vc - 1).unwrap(), pos_equal_eps)
                    {
                        result
                            .get_or_insert_with(|| {
                                Self::OutputPolyline::from_iter(
    ...
    fn rotate_start(
        ...
        let start_v = self.get(start_index).unwrap();
        if start_v.pos().fuzzy_eq_eps(point, pos_equal_eps) {
            ...
            if point.fuzzy_eq_eps(self.get(next_index).unwrap().pos(), pos_equal_eps) {
                result.reserve(vc);
                ...
                let split = seg_split_at_point(
                    self.get(start_index).unwrap(),
                    self.get(next_index).unwrap(),
                    point,
                    pos_equal_eps,
                );
    ...
    fn closest_point(&self, point: Vector2<Self::Num>) -> Option<ClosestPointResult<Self::Num>> {
        ...
        let mut result = ClosestPointResult {
            seg_start_index: 0,
            seg_point: self.get(0).unwrap().pos(),
        ...
        for (i, j) in self.iter_segment_indexes() {
            let v1 = self.get(i).unwrap();
            let v2 = self.get(j).unwrap();
...
pub trait PolylineRefMut<'a>:
    PolylineRef<'a> + std::ops::Index<usize, Output = PlineVertex<Self::Num>>
{
    ...
    fn invert_direction_mut(&mut self) {
        ...
        let mut end = vc - 1;
        while start < end {
            let s = self.get(start).unwrap();
            let e = self.get(end).unwrap();
        ...
        let first_bulge = self.get(0).unwrap().bulge;
        for i in 1..vc {
            let b = -self.get(i).unwrap().bulge;
            self.set_vertex(i - 1, self.get(i - 1).unwrap().with_bulge(b));
        }

        if self.is_closed() {
            self.set_vertex(vc - 1, self.get(vc - 1).unwrap().with_bulge(-first_bulge));
        }
    }
}
    ...
    T: Real,
{
    ...
    fn fuzzy_eq_eps(&self, other: &Self, fuzzy_epsilon: T) -> bool {
