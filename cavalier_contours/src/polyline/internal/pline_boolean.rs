use crate::{
    ...
    polyline::{
        ...
        PlineBooleanOptions, PolylineCreation, PolylineRef, PolylineSlice,
    },
};
        ...
        loop {
            ...
            if let Some(next_intr_list) = intersects_lookup.get(&index) {
                ...
                let midpoint = seg_midpoint(
                    slice.updated_start,
                    pline.at(pline.next_wrapping_index(slice.start_index)),
                );
                if point_on_slice_pred(midpoint) {
    ...
    O: PolylineCreation<Num = T>,
{
    ...
    let slice_to_pline = |s: &BooleanPlineSlice<T>| {
        if s.source_is_pline1 {
            s.to_polyline(source_pline1, pos_equal_eps)
        } else {
            s.to_polyline(source_pline2, pos_equal_eps)
        }
    };

    let stitch_slice_onto = |s: &BooleanPlineSlice<T>, target: &mut O| {
        if s.source_is_pline1 {
            s.stitch_onto(source_pline1, target, pos_equal_eps);
        } else {
            s.stitch_onto(source_pline2, target, pos_equal_eps);
        }
    };
    ...
    match operation {
        BooleanOp::Or => {
            if boolean_info.completely_overlapping() {
                ...
                BooleanResult::from_whole_plines(vec![O::clone_from(pline2)], Vec::new())
                ...
                if is_pline1_in_pline2() {
                    ...
                    BooleanResult::from_whole_plines(vec![O::clone_from(pline1)], Vec::new())
                } else {
                    BooleanResult::from_whole_plines(
                        vec![O::clone_from(pline1), O::clone_from(pline2)],
                        Vec::new(),
                    )
                }
            } else {
        ...
        BooleanOp::Not => {
            if boolean_info.completely_overlapping() {
                ...
                if is_pline1_in_pline2() {
                    ...
                    BooleanResult::from_whole_plines(
                        vec![O::clone_from(pline1)],
                        vec![O::clone_from(pline2)],
                    )
                } else {
                    // disjoint
                    BooleanResult::from_whole_plines(vec![O::clone_from(pline1)], Vec::new())
